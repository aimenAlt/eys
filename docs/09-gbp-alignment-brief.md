...
### 7c. GA4 events
`src/utils/analytics.ts` has a solid named-event scaffold. Verify these actually fire on the new pages: `phone_click`, `jobber_booking_click`, `lead_submit`, `booking_complete`. (`estimate_form_open` was removed with the Jobber embed — the embedded funnel it measured no longer exists; outbound estimate clicks are now `jobber_booking_click` with `booking_type=project_estimate`.)
